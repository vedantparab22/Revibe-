...
const MessageContent = ({ text }: { text: string }) => {
  // Enhanced parsing for bold markdown **text** and handling of line breaks
...
const ChatInterface: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: "I'm having trouble connecting to my knowledge base. Please try again in a moment." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-180px)] flex flex-col bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
      ...
      {/* Chat History */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-opacity-5 scroll-smooth">
        {messages.map((msg, i) => (
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="flex gap-3 max-w-[80%]">
              ...
              <div className="p-4 rounded-[24px] bg-slate-100 rounded-tl-none flex items-center gap-1 shadow-sm border border-slate-200">
