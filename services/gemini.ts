...
export const getQuickAdvice = async (query: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      ...
      contents: query,
      config: {
        systemInstruction: "You are ReVibe, an AI-powered e-waste advisor for India. Keep answers concise, helpful, and focused on Indian e-waste rules and sustainability. Use emojis. Use **bold text** for key terms, names of platforms, or important actions.",
      }
    });
